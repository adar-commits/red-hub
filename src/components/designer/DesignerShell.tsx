...
function LogoutButton() {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 w-full px-3 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10"
    >
      התנתק
    </button>
  );
...
export function DesignerShell({
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[var(--background)]">
      {/* Sidebar — desktop */}
      <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 md:rtl:right-0 md:rtl:left-auto bg-[var(--sidebar-bg)] text-white">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  active ? "bg-[var(--brand-red)] text-white" : "text-white/90 hover:bg-white/10"
                }`}
              >
                <span>{item.icon}</span>
                {item.label}
      ...
      {/* Bottom nav — mobile */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[var(--sidebar-bg)] text-white border-t border-white/10 safe-area-pb flex justify-around py-2">
        <Link
          href="/dashboard"
          className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs ${pathname === "/dashboard" ? "text-[var(--brand-red)]" : "text-white/80"}`}
        >
          <span>🏠</span>
          <span>בית</span>
        </Link>
        <Link
          href="/deals"
          className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs ${pathname === "/deals" ? "text-[var(--brand-red)]" : "text-white/80"}`}
        >
          <span>🤝</span>
          <span>עסקאות</span>
        </Link>
        <Link
          href="/commissions"
          className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs ${pathname === "/commissions" ? "text-[var(--brand-red)]" : "text-white/80"}`}
        >
          <span>💰</span>
          <span>עמלות</span>
        </Link>
        <Link
          href="/business"
          className={`flex flex-col items-center gap-0.5 px-3 py-1 text-xs ${pathname === "/business" ? "text-[var(--brand-red)]" : "text-white/80"}`}
        >
          <span>📋</span>
          <span>פרטים</span>
        </Link>
